...
public class SwerveDrive extends Subsystem implements PIDSource, PIDOutput {
	...
	Joystick joy1;
	Gyro gyro;
	// PWMSpeedController flDrive, frDrive, blDrive, brDrive, flsteer, frsteer,
	// blsteer, brsteer;
//	PIDController pid;
	double lastAngle;
	...
	public SwerveDrive(Gyro gyro) {
		...
		// Initializes and defines Swerve Modules. The Array can be of any size, so long
		// as they are all defined
		modules = new SwerveModule[] {
				...
				new SwerveModule(new WPI_TalonSRX(Constants.SwerveDrive.FL_DRIVE),
						...
						-Constants.SwerveDrive.WHEEL_BASE_WIDTH / 2, Constants.SwerveDrive.WHEEL_BASE_LENGTH / 2),
	...
	/**
	 * @param pivotX x coordinate in inches of pivot point relative to center of
	 *               robot
	 * @param pivotY y coordinate in inches of pivot point relative to center of
	 *               robot
	 */
	...
	private void driveWithOrient(double translationX, double translationY, double rotation, boolean fieldOrientation) {
		...
		Vector2D transVect = new Vector2D(translationX, translationY), pivotVect = new Vector2D(pivotX, pivotY);
		setTrans(transVect);
...
//			this.joy1 = joy1;
//			double x = (joy1.getRawAxis(Constants.Xinput.LX_AXIS));
//			double y = (joy1.getRawAxis(Constants.Xinput.LY_AXIS));
//			double z = (joy1.getRawAxis(Constants.Xinput.RX_AXIS));
//			speed = 100;
//			turnRate = 100;
//
//			if (joy1.getRawButton(Constants.Xinput.LEFT_BUMPER)) {
//				turnRate = 20;
//				speed = 20;
//			}
//			if (joy1.getRawButton(Constants.Xinput.RIGHT_BUMPER)) {
//				lockWheels();
//			}
//			if (joy1.getRawButton(Constants.Xinput.JOYR_PRESS)) {
//				gyro.reset();
//			}
//			if (joy1.getRawButton(Constants.Xinput.JOYL_PRESS)) {
//				if (!ypressed) {
...
//		else {
			...
			if (joy1.getRawButton(Constants.DirectInput.A)) {
				lockWheels();
			}
			if (joy1.getRawButton(Constants.DirectInput.LEFT_TRIGGER)) {
				turnRate = 20;
				speed = 35;
			} else if (joy1.getRawButton(Constants.DirectInput.RIGHT_TRIGGER)) {
				turnRate = 100;
				speed = 100;
			}
			if (joy1.getRawButton(Constants.DirectInput.RIGHT_BUMPER))
				gyro.reset();

			if (joy1.getRawButton(Constants.DirectInput.LEFT_BUMPER)) {
				if (!ypressed)
					drivingField = !drivingField;
				ypressed = true;
				SmartDashboard.putBoolean("field?", drivingField);
